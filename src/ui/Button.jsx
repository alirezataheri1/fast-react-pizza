...
function Button({ children, isSubmitting, to, type, onClick }) {
  ...
  const style = {
    ...
    round: baseStyle + " text-sm px-2.5 py-1 md:px-3.5 md:py-2",
    secondary:
  ...
  if (onClick) {
    return (
      <button className={style[type]} onClick={onClick} disabled={isSubmitting}>
        {children}
      </button>
    );
  }
  ...
  return (
    <button className={style[type]} disabled={isSubmitting}>
      {children}
    </button>
  );
}
